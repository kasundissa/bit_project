...
class grn{
    ...
    function register() //to enter grn information in to the system
    {
        $sql="insert into grn(ref_no,sup_id,total_cost) values('$this->ref_no','$this->supplier','$this->total_cost')"; //insert grn information in database
        $this->db->query($sql);
        $gid=$this->db->insert_id; //to get grn id


        $c=0;
        $d=0;
        foreach ($this->drug_name as $item){ //insert drugs information for every grn
        ...
        values('".$this->drug_name[$c]."','".$this->pack_size[$c]."','".$this->no_of_packs[$c]."','".$this->batch_no[$c]."','".$this->weight[$c]."','".$this->manufacture_date[$c]."','".$this->expire_date[$c]."','".$this->unit_price[$c]."','".$this->selling_price[$c]."','".$this->cost[$c]."','".$gid."')";
            ...
            $sql3 = "insert into stock(ref_type,ref_no,item_id,st_in,st_out) values ('GRN','".$gid."','".$this->drug_name[$c]."','".$qty[$d]."','0')"; //calculate drug stock
            $this->db->query($sql3);
            // echo $sql3;
            $c++;
            $d++;
    ...
    function getall() //get information for grn report
    {
        $filter="";
    ...
    function get_by_id($id) //to view grn by reference no
    {
    ...
    function get_amt($AMT) //to get drug price by drug id
    {
        $sql = "select * from drug_product where drg_name=$AMT order by pro_id desc limit 1";
        $res = $this->db->query($sql);
        $row = $res->fetch_array();
